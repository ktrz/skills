...
- Suppress already-actioned review asks. Phase 3 gains a "Resolve review state for referenced PRs" sub-step: after Slack dedup, fenced message bodies are scanned for PR references (`github.com/<owner>/<repo>/pull/<N>` URLs, plus `#N` only with repo context), mapped to `(repo, N, message_ts)`, and review state for those PRs plus all `reviewRequested` PRs is fetched in one `gh api graphql` batch (`state`, `commits(last: 1) { commit { committedDate } }`, `reviews(last: 20) { author { login }, state, submittedAt }`, `viewer { login }` → `ME_LOGIN`) — no new `allowedTools` entry needed. Phase 4 gains a state-aware suppression rule keyed on whether my latest review covers the PR's current head, not on ask timing: a "Review PR #N" bullet is dropped when the PR is merged/closed or my latest review is `APPROVED` (later fix-up commits don't need re-review; a dismissed approval self-heals via `reviewRequested`); kept in "Do first" when my latest review is `CHANGES_REQUESTED` and new commits landed after it (author claims fixed); moved to a quiet "Already handled" sub-note when my review already covers the head commit or was only `COMMENTED`; anything uncertain (no review, lookup error) stays in "Do first". `message_ts` is now cosmetic (note wording only), so multiple Slack mentions of the same PR no longer risk suppressing the wrong bullet. Trust boundaries table gains a row for review author logins, states + timestamps (structured, compared not quoted, LOW).
