...
3. **Present options** — based on your investigation, offer the user concrete choices. Tailor these to the specific comment, but common options include:
   - **Fix it** — describe the specific change you'd make (e.g., "I can rename this variable to `userCount` and update the 3 references")
...
5. **Implement the chosen action**:
   ...
   - Commit the fix with a descriptive message referencing the ticket if one is apparent from the branch name. Always review `git diff` before committing.
