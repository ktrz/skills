...
`skills/review/resolve-pr-comments/references/`. Do not duplicate those queries here —
...
`skills/review/review-pr/references/aggregation.md`), with the resolved body staying
...
`skills/review/resolve-pr-comments/references/investigate.md` verbatim. Each
...
- Subagent returns one structured report per item in input order, per
  the format defined in `skills/review/resolve-pr-comments/references/investigate.md`.
...
- **GraphQL fetch is not duplicated** — the paginated GitHub queries live
  in `skills/review/resolve-pr-comments/references/`. Load and reuse; do not
  re-implement.
- **Handover format is the single source of truth** — all fields written
  ...
  itself schema-compatible with `skills/review/review-pr/references/findings-schema.md`.
