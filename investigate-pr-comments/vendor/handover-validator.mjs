#!/usr/bin/env node
// GENERATED by 'npm run build' (esbuild bundle of cli.ts + vendor/*.ts + zod). Do not edit by hand.
...
// vendor/types.ts
var SeveritySchema = external_exports.enum(["critical", "important", "suggestion", "nit"]);
var StatusMarkerSchema = external_exports.enum(["unresolved", "resolved", "custom", "deferred", "skipped"]);
var LocationSchema = external_exports.discriminatedUnion("kind", [
  external_exports.object({ kind: external_exports.literal("review-body") }),
  external_exports.object({ kind: external_exports.literal("file"), file: external_exports.string(), line: external_exports.number().int().positive() })
]);
var SourceSchema = external_exports.discriminatedUnion("kind", [
  external_exports.object({ kind: external_exports.literal("auto-review"), severity: SeveritySchema }),
  external_exports.object({
    kind: external_exports.literal("reviewer"),
    // Bare handle; serializer prepends @
    login: external_exports.string().min(1).regex(/^[^\s@]+$/),
...
var FindingItemBaseSchema = external_exports.object({
  id: external_exports.string().min(1),
  status: StatusMarkerSchema,
  source: SourceSchema,
  location: LocationSchema,
  reportedBy: external_exports.array(external_exports.string().min(1)).nonempty(),
  ...
  chat: external_exports.array(ChatMessageSchema).optional()
});
var FindingItemSchema = external_exports.discriminatedUnion("dirty", [
  FindingItemBaseSchema.extend({ dirty: external_exports.literal(false), rawSource: external_exports.string().min(1) }),
  FindingItemBaseSchema.extend({ dirty: external_exports.literal(true), rawSource: external_exports.string().optional() })
...
var BranchRefSchema = external_exports.object({
  ref: external_exports.string().brand(),
  sha: external_exports.string().brand().optional()
});
var DocumentHeaderSchema = external_exports.object({
  prUrl: external_exports.string().url().brand(),
...
var HandoverDocumentSchema = external_exports.object({
  header: DocumentHeaderSchema,
  items: external_exports.array(FindingItemSchema)
});

// vendor/parse.ts
var ParseError = class extends Error {
  constructor(message, offset, state, lineNumber, options) {
...
var ITEM_HEADING_RE = /^## \[([?x~d\-])\] (auto:([a-z]+)|reviewer:@([^\s@]+)) — (review body|([^\s:]+):(\d+))$/;
var FIELD_RE = /^\*\*([^*]+):\*\*\s*(.*)/;
var OPTION_BULLET_RE = /^\s*[-*]\s+(.+)/;
function parseSourceCounts(line) {
  const m = line.match(
...
var KNOWN_ITEM_FIELDS = /* @__PURE__ */ new Set([
  "Severity",
  "Source",
  "Reported by",
  "Id",
...
var CHAT_BULLET_RE = /^- ([a-zA-Z]+): (.*)$/;
var CHAT_CONTINUATION_RE = /^  (.*)$/;
var EXTERNAL_DATA_OPEN_RE = /^<external_data\s+(?:[^>]*\s)?trust="untrusted"(?:\s[^>]*)?>$/;
var EXTERNAL_DATA_ANY_OPEN_RE = /^<external_data(?:\s[^>]*)?>$/;
var EXTERNAL_DATA_CLOSE_RE = /^<\/external_data>$/;
function handleItemBoundary(line) {
  if (line.trim() === "---") {
    return "separator";
  }
  if (line.startsWith("## [")) {
...
function flushCommentBody(bodyLines, offset, lineNum) {
  let start = 0;
...
function finalizeItem(item, raw, endOffset, out) {
  ...
  const finalized = {
    ...
    rawSource,
    dirty: false,
    ...item.chat !== void 0 && item.chat.length > 0 ? { chat: item.chat } : {}
  };
  out.push(finalized);
}
function applyField(item, key, value, offset, lineNum, state) {
...
function parseItemHeading(line, offset, lineNumber, state, startOffset) {
  ...
  return {
    ...
    resolution: "",
    chat: void 0,
    severitySeen: source.kind === "auto-review",
...
function parseDocument(raw) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (sv.state === "IN_HEADER") {
      ...
      const commentItem = sv.item;
      const fenceOpen = sv.fenceOpen;
      const bodyLines = sv.bodyLines;
      ...
      if (boundary !== null) {
        ...
        if (fieldMatch) {
          ...
          if (bulletMatch) {
            ...
            if (sv.item.chat === void 0) {
              sv.item.chat = [];
            }
            sv.item.chat.push({ role, content });
          } else {
            const contMatch = CHAT_CONTINUATION_RE.exec(line);
            if (contMatch && sv.item.chat !== void 0 && sv.item.chat.length > 0) {
...
${contMatch[1]}`;
      ...
      if (boundary !== null) {
        ...
        sv.item.resolution = sv.item.resolution ? `${sv.item.resolution}
${line}` : line;
      }
    }
  ...
  if (sourceCounts !== null) {
    ...
    const sugNitCount = items.filter(
      (it) => it.source.severity === "suggestion" || it.source.severity === "nit"
    ).length;
    const totalCount = items.length;
    if (sourceCounts.totalItems !== totalCount || sourceCounts.autoReviewFindings !== autoCount || sourceCounts.humanReviewerComments !== humanCount || sourceCounts.totalCritical !== critCount || sourceCounts.totalImportant !== impCount || sourceCounts.totalSuggestionOrNit !== sugNitCount) {
      throw new ParseError(
  ...
  const ParseTimeFindingItemSchema = external_exports.discriminatedUnion("dirty", [
    FindingItemBaseSchema.extend({
      id: external_exports.string(),
      dirty: external_exports.literal(false),
      rawSource: external_exports.string().min(1)
    }),
    FindingItemBaseSchema.extend({
      id: external_exports.string(),
      dirty: external_exports.literal(true),
      rawSource: external_exports.string().optional()
    })
  ]).superRefine((data, ctx) => {
    if (data.status === "resolved" || data.status === "custom") {
      if (!data.resolution.length) {
  ...
  const ParseTimeDocSchema = external_exports.object({
    header: DocumentHeaderSchema,
    items: external_exports.array(ParseTimeFindingItemSchema)
  });
  try {
