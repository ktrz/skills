...
Dispatch an Agent (model inherits sonnet from the subagent default) with `run_in_background: false` — you need the findings before writing the plan. Use the actual ticket summary and description from Step A — do not guess or paraphrase.
...
- **Yes** → invoke `grill-me`. Seed it with the ticket description, codebase findings from Stage 1, architecture.md constraints, and any open questions from the ticket. Grill-me will ask one question at a time and can explore the codebase itself. Proceed to Stage 3 when requirements feel solid.
